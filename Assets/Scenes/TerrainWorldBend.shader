        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                fixed4 TriplanarSample(sampler2D tex, float3 worldPos, float3 blendAxes)
                {
                    ...
                    return xSample * blendAxes.x + ySample * blendAxes.y + zSample * blendAxes.z;
                }
                ...
                fixed4 frag(v2f i) : SV_Target
                {
                    ...
                    blendAxes = blendAxes / (blendAxes.x + blendAxes.y + blendAxes.z);

                    float height = i.originalWorldPos.y;

                    float slope = worldNormal.y;

                    float flatWeight = smoothstep(_SlopeThreshold - _SlopeBlend, _SlopeThreshold + _SlopeBlend, slope);
                    ...
                    float h2Weight = smoothstep(_Height1 - _HeightBlend, _Height1, height) - smoothstep(_Height2 - _HeightBlend, _Height2, height);
                    float h3Weight = smoothstep(_Height2 - _HeightBlend, _Height2, height);

                    fixed4 flatColor = flatTex1 * h1Weight + flatTex2 * h2Weight + flatTex3 * h3Weight;

                    fixed4 steepColor = TriplanarSample(_SteepTex, i.originalWorldPos, blendAxes);

                    fixed4 finalColor = flatColor * flatWeight + steepColor * steepWeight;

                    // Apply smooth inversion for night mode
                    finalColor.rgb = lerp(finalColor.rgb, 1.0 - finalColor.rgb, _Invert);
